...
if __name__ == "__main__":
    ...
    llr_NB = functions2.compute_NB_llrs(DTROriginalNormalized, LTR, DTEOriginalNormalized, labels)
    ...
    with open("datiMVG/datiZscoreNB.txt", "w") as fp:
        fp.write(toPrint)
    ...
    llr_NB = functions2.compute_NB_llrs(DTROriginal, LTR, DTEOriginal, labels)
    for threshold in thresholds:
        optimalBayesDecisionPredictions = functions2.compute_optimal_bayes_decision_given_threshold(llr_NB, threshold)
        confusionMatrix = functions2.compute_binary_confusion_matrix(optimalBayesDecisionPredictions, LTE)
        DCFsNormalized1.append(
    ...
    with open("datiMVG/datiRawNB.txt", "w") as fp:
        fp.write(toPrint)